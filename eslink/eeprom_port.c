...
//读脱机文件名   
error_t get_offline_proget_name(uint8_t *buf, uint8_t size)
{
    uint8_t ret;
...
error_t set_eslinkii_serial_number(uint8_t *buf, uint8_t size)
{
    ...
    ret = fm24cxx_write(EE_ESLINK_SERIAL_NUMBER_ADDR, buf, size); 
    if(ret != TRUE)
        return ERROR_EE_WRITE;
    return ERROR_SUCCESS;
}
...
{
    uint8_t ret;
    ret = fm24cxx_read(EE_ESLINK_SERIAL_NUMBER_ADDR, buf, size);
