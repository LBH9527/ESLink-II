...
static void flash_data_write_set(uint32_t data)
{
    uint8_t byte[4];
...
uint8_t isp_erase_chip(void)
{
    uint8_t result;  
    //擦除code区
    result = erase_and_check(ERASE_M0_CMD,FLASH_MAIN_AREA, 15);
    if(result != TRUE)
        return FALSE;
    //擦除info0区
    result = erase_and_check(ERASE_M0_CMD, FLASH_INFO_AREA, 5);
    if(result != TRUE)
        return FALSE;  
//   
