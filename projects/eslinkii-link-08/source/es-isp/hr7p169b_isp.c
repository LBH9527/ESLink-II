...
//0xE3 设置缓冲器地址
static void addr_set(uint16_t addr)
{
    uint8_t byte[2];

    byte[0] = (addr & 0xFF00) >> 8;
    byte[1] = (addr & 0x00FF) ;
    isp_send_bytes(ADDR_SET_CMD, byte, sizeof(byte));   
}
//0xE4   设置数据缓冲器
static void data_write_set(uint16_t data)
{
    uint8_t byte[2];

    byte[0] = (data & 0x0000FF00) >> 8;
    byte[1] = (data & 0x000000FF) ;
    isp_send_bytes(DATA_WRITE_CMD, byte, sizeof(byte)); 
...
{
    uint8_t i;
    uint8_t unlock_code[19] = {0x55,0xAA,0x49,0x63, 0x68,0x61,0x69,0x65, 0x72,0x20,0x43,0x6f,
                                0x2c,0x4c,0x74,0x64, 0x2E, 0xAA, 0x55 };     
...
//    byte[0] = (ID_CHECK_CMD & 0x000000FF) ;
    ...
    for(i = 0; i < 2; i++)
    {
        byte_rcv[i] = isp_readb();
    }       
    isp_end_bit(); 
    for(i = 0; i < 2; i++)
    {
        if(byte_rcv[i] != ((ID_CHECK_VAL >> i*8) & 0xff ) )
             return FALSE;
    }         
    return TRUE;
}
...
* 输出 ：
*******************************************************************************/
static uint8_t read_data(uint16_t addr, uint16_t *data, uint16_t size) 
{
     uint32_t i;
     uint8_t tmp_data[2];
    ...
    for( i=0; i<size; i++)
    {
         isp_rcv_bytes(FLASH_READ_PLUS_CMD, tmp_data, 2);   
         *data = (  (tmp_data[0] << 8) |
                    (tmp_data[1] ) );
         data++;               
    } 
...
uint8_t isp_erase_chip(void)
{
    ...
    result = erase_and_check(ERASE_OP0,OPTION_0_AREA,10); 
    if(result != TRUE)
        return FALSE;    
