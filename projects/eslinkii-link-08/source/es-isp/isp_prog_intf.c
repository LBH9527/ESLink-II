...
static error_t isp_prog_read_chip_chksum(uint8_t *buf)
{
    ...
    if(isp_read_config(CHIP_CHECKSUM_ADDR, (uint16_t*)buf, 2) != TRUE)
        return ERROR_ISP_READ_CFG_WORD;
...
static error_t isp_prog_program_flash(uint32_t addr, uint8_t *data, uint32_t size, uint32_t *failed_addr)
{
    ...
    addr = addr/2;
    ret = isp_program_code((uint16_t) addr, (uint16_t*)data, size_in_words,&offset);
    if(ret != TRUE)
    {
        if( failed_addr)
...
static error_t isp_prog_read_flash(uint32_t addr, uint8_t *data, uint32_t size)
{
    ...
    size_in_words = size/2;  
    addr = addr/2;    
    if(isp_read_code(addr, (uint16_t*)data, size_in_words) != TRUE)
...
{   
    ...
    ret = isp_program_config(addr/2, (uint16_t*)data, size_in_words, &offset);
    if(ret != TRUE)
    {
        if(failed_addr)
            *failed_addr =  addr + offset;
...
static error_t isp_prog_read_config(uint32_t addr,  uint8_t *buf, uint32_t size)
{
    ...
    if(size & 0x01)
        return ERROR_OUT_OF_BOUNDS;
    size_in_words = size/2;    
    if(isp_read_config(addr, (uint16_t*)buf, size_in_words) != TRUE)
        return ERROR_ISP_READ_CFG_WORD;
    return ERROR_SUCCESS;        
}
/*
...
{ 
    ...
    if(ERROR_SUCCESS != ret)
        return ret; 
    if(isp_program_config(isp_target_dev->encrypt_addr, (uint16_t *)&isp_target_dev->encrypt_value, 1, NULL) != TRUE)
