...
static uint32_t target_flash_erase_sector_size(uint32_t addr)
{
    if(swd_target_device.sector_info_length > 0) { 
        int sector_index = swd_target_device.sector_info_length - 1;
        for (; sector_index >= 0; sector_index--) {
            if (addr >= swd_target_device.sectors_info[sector_index].start) {
                return swd_target_device.sectors_info[sector_index].size;
            }
        }
    }
    return swd_target_device.sector_size;
}
