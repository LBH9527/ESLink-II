...
//error_t swd_flash_manager_data()
//{
//    uint32_t start_addr;
//    uint32_t end_addr;
//    uint32_t addr;
...
//    }
//    
//    start_addr = target_device.flash_start;
...
//        status = setup_next_sector(start_addr);
...
//    
//    while (true) {
//        spi_flash_read(SF_USER_HEX_ADDR + current_write_block_addr, current_write_block_size, hex_buf);
//        // flush if necessary
//        if (end_addr >= current_write_block_addr + current_write_block_size) {
//            // Write out current buffer
//            status = intf->program_page(current_write_block_addr, hex_buf, current_write_block_size);
...
//            // Setup for next page
//            memset(hex_buf, 0xFF, current_write_block_size);
//            buf_empty = true;
//            current_write_block_addr += current_write_block_size;
//        }

//        // Change sector if necessary
//        if (end_addr >= current_sector_addr + current_sector_size) {
