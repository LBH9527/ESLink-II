...
uint8_t icd_control(uint8_t cmd_data)
{
    ...
    {
        icd_start();
        check_ack = _write_cmd_read_ack(cmd_data);
        icd_end();   
        if(check_ack == TRUE)
            break;           
    }   
...
static uint8_t icd_run(void)
{
	...
	if(icd_control(RUN_FLAG_CMD) != ICD_ERR_NONE) 
        return CHIP_CONTROL_ERROR;
...
//		return CHIP_CONTROL_ERROR;
    ...
    if(icd_write_data_operation(STPWHL_REG_ADDR,0)!= ICD_ERR_NONE)       //stopwatch 清零
        return CHIP_CONTROL_ERROR;
	...
	{     
		if(icd_breakpoint.addr_buffer[i] != 0x00)
		{
            ...
            if(icd_write_data_operation(SPCBK_REG_ADDR,icd_breakpoint.addr_buffer[i])!= ICD_ERR_NONE)  
                return CHIP_CONTROL_ERROR;
		}		
    }
...
uint8_t emu_C_step(uint8_t *wr_buf)	
{
    ...
    {
        if(icd_read_data_operation(PCR_REG_ADDR, &pc_value ) != ICD_ERR_NONE)    //复位后pc为0，c单步直接到main 20111206
            return CHIP_CONTROL_ERROR;
        if(pc_value!=0)
            break;     
    }
    if(pc_value == 0)
	{
        if(icd_write_data_operation(CDBK_REG_ADDR,icd_main.addr)!= ICD_ERR_NONE) //set breakpoint
            return CHIP_CONTROL_ERROR;
...
//需要汇编单步走过的起始地址和结束地址
    ...
    if(icd_write(SET_ADDR,LSTPCR_REG_ADDR) != ICD_ERR_NONE) 
        return CHIP_CONTROL_ERROR;
...
//            return CHIP_CONTROL_ERROR;
        ...
        for(i=0;i<=2;i++)		
        {
            if(icd_read_data_operation(PCR_REG_ADDR, &pc_value ) != ICD_ERR_NONE)    // set sram initial address		
                return CHIP_CONTROL_ERROR;
            if(pc_value!=0)
                break;     
        }
...
{    
    ...
    if(icd_write_data_operation(STPWHL_REG_ADDR, 0x0000) != ICD_ERR_NONE)	//stopwatch 清零
        return CHIP_CONTROL_ERROR;
...
//			break;
//		}
        if(icd_read_data_operation(PCR_REG_ADDR, &pc_value ) != ICD_ERR_NONE)    
                return CHIP_CONTROL_ERROR;
...
//------------------------------------------------------------ // write PC
    ...
    if(icd_write_data_operation(PCR_REG_ADDR,pc2.B16)  != ICD_ERR_NONE)
        return CHIP_CONTROL_ERROR;
...
{   
    ...
    if(icd_read_data_operation(PCR_REG_ADDR, &pc.B16 ) != ICD_ERR_NONE)
        return CHIP_CONTROL_ERROR;
    buf[i++] = 0;
    buf[i++] = 0;
    buf[i++] = pc.B08[1];
    ...
    if(icd_read_data_operation(LSTPCR_REG_ADDR, &pc.B16 ) != ICD_ERR_NONE)
        return CHIP_CONTROL_ERROR;
    buf[i++] = 0;
    buf[i++] = 0;
    buf[i++] = pc.B08[1];
...
//*******************************************************************/
//设置PC断点
uint8_t emu_set_breakpoint(uint8_t *wr_buf)		//
{   
	uint8_t i;
    ...
    if(i >= BreakPointNum)
        return CHIP_CONTROL_ERROR;    
    if(icd_write_data_operation(SPCBK_REG_ADDR, bkpt_addr.B16)  != ICD_ERR_NONE)
        return CHIP_CONTROL_ERROR;
...
/********************************************************************/
// 清除断点
uint8_t emu_clr_breakpoint(uint8_t *wr_buf)		
{	
	uint8_t i;
    union B16_B08 bkpt_addr;
	...
	if(icd_write_data_operation(CPCBK_REG_ADDR, bkpt_addr.B16)!= ICD_ERR_NONE)  //清除断点
        return CHIP_CONTROL_ERROR;
...
uint8_t emu_clr_all_breakpointer(void)  // clr  all breakpointer
{
	...
	if(icd_write_data_operation(CPCBK_REG_ADDR, 0x0000) != ICD_ERR_NONE)	// clr  all breakpoint
        return CHIP_CONTROL_ERROR;
...
uint8_t update_chip_status(uint8_t *rd_buf, uint16_t *read_len)				                 
{
    ...
    //a reg
//    if(icd_read_data_operation(AREG_REG_ADDR, &read_data.B16) != ICD_ERR_NONE) 
    if(icd_read_data_operation(0xFF85, &read_data.B16) != ICD_ERR_NONE)    //0xFF85 累加器A寄存器
        return CHIP_CONTROL_ERROR;
    rd_buf[ICD_DATA_OFFSET] = read_data.B08[0];
    //b reg
    rd_buf[ICD_DATA_OFFSET + 1] = 0;               //7P196 no b reg
    //current pc
    if(icd_read_data_operation(PCR_REG_ADDR, &read_data.B16) != ICD_ERR_NONE)    //9527
        return CHIP_CONTROL_ERROR;
    ...
    //last pc
    if(icd_read_data_operation(LSTPCR_REG_ADDR, &read_data.B16) != ICD_ERR_NONE)    //9527
        return CHIP_CONTROL_ERROR;
    ...
    //STOPWATCH
    if(icd_read_data_operation(STPWHH_REG_ADDR, &read_data.B16) != ICD_ERR_NONE)    //9527
        return CHIP_CONTROL_ERROR;
    rd_buf[ICD_CODE_OFFSET + 8] = read_data.B08[1];
    rd_buf[ICD_CODE_OFFSET + 8] = read_data.B08[0];
    if(icd_read_data_operation(STPWHL_REG_ADDR, &read_data.B16) != ICD_ERR_NONE)    //9527
        return CHIP_CONTROL_ERROR;
...
//读已用堆栈内容
uint8_t emu_read_used_stack(uint8_t *rd_buf, uint16_t *read_len)					//display all levels stack //need modify
{
    ...
    if(icd_read_data_operation(STKLV_REG_ADDR, &read_data.B16) != ICD_ERR_NONE)
        return CHIP_CONTROL_ERROR;
	...
	{
        if(icd_read_data_operation(STK0_REG_ADDR+i, &read_data.B16) != ICD_ERR_NONE)
            return CHIP_CONTROL_ERROR;
...
//读全部堆栈内容
uint8_t emu_read_all_stack(uint8_t *rd_buf, uint16_t *read_len)			//display all levels stack //need modify
{
    ...
    {
        if(icd_read_data_operation(STK0_REG_ADDR + i, &stack_data.B16) != ICD_ERR_NONE)
            return CHIP_CONTROL_ERROR;
        rd_buf[ICD_CODE_OFFSET + i*4] = 0;
        rd_buf[ICD_CODE_OFFSET + i*4 + 1] = 0;    
...
uint8_t emu_set_condition_breakpoint(uint8_t *wr_buf)
{
    ...
    if(icd_write_data_operation(BKS_REG_ADDR, condition_bkpt.B16)  != ICD_ERR_NONE)
        return CHIP_CONTROL_ERROR;	
    
	condition_bkpt.B08[1] = wr_buf[i++];  // condition registerhigh8bit
	condition_bkpt.B08[0] = wr_buf[i++];  // condition register low8bit
    if(icd_write_data_operation(STBK_REG_ADDR, condition_bkpt.B16)  != ICD_ERR_NONE)
        return CHIP_CONTROL_ERROR;	
	
    condition_bkpt.B08[1] = wr_buf[i++];  // condition registerhigh8bit
	condition_bkpt.B08[0] = wr_buf[i++];  // condition register low8bit
    if(icd_write_data_operation(ABK_REG_ADDR, condition_bkpt.B16)  != ICD_ERR_NONE)
        return CHIP_CONTROL_ERROR;	
  	
    condition_bkpt.B08[1] = wr_buf[i++];  // condition registerhigh8bit
	condition_bkpt.B08[0] = wr_buf[i++];  // condition register low8bit
    if(icd_write_data_operation(RBKA_REG_ADDR, condition_bkpt.B16)  != ICD_ERR_NONE)
        return CHIP_CONTROL_ERROR;	  

    condition_bkpt.B08[1] = wr_buf[i++];  // condition registerhigh8bit
	condition_bkpt.B08[0] = wr_buf[i++];  // condition register low8bit
    if(icd_write_data_operation(RBKD_REG_ADDR, condition_bkpt.B16)  != ICD_ERR_NONE)
        return CHIP_CONTROL_ERROR;	  
    
    condition_bkpt.B08[1] = wr_buf[i++];  // condition registerhigh8bit
	condition_bkpt.B08[0] = wr_buf[i++];  // condition register low8bit
    if(icd_write_data_operation(PCBKT_REG_ADDR, condition_bkpt.B16)  != ICD_ERR_NONE)
        return CHIP_CONTROL_ERROR;	    
    
    condition_bkpt.B08[1] = wr_buf[i++];  // condition registerhigh8bit
	condition_bkpt.B08[0] = wr_buf[i++];  // condition register low8bit
    if(icd_write_data_operation(RBKT_REG_ADDR, condition_bkpt.B16)  != ICD_ERR_NONE)
        return CHIP_CONTROL_ERROR;	
    return ICD_ERR_NONE	;    
}
